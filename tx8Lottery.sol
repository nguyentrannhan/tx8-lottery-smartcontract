...
// File: contracts/interfaces/ITX8Lottery.sol

pragma solidity ^0.8.4;

interface ITX8Lottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in TX8 per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in TX8 token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInTX8: price of a ticket in TX8
 ...
 */
contract TX8Lottery is ReentrancyGuard, ITX8Lottery, Ownable {
    ...
    uint256 public maxpriceTicketInTX8 = 50 ether;
    uint256 public minpriceTicketInTX8 = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInTX8;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] tx8PerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInTX8;
        uint32 finalNumber;
    }
     ...
     * @param _tx8TokenAddress: address of the TX8 token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _tx8TokenAddress, address _randomGeneratorAddress) {
        tx8Token = IERC20(_tx8TokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of TX8 to this contract
        uint256 amountTX8ToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInTX8,
            _ticketNumbers.length
        );

        // Transfer TX8 tokens to this contract
        tx8Token.safeTransferFrom(address(msg.sender), address(this), amountTX8ToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInTX8 += amountTX8ToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInTX8ToTransfer
        uint256 rewardInTX8ToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInTX8ToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        tx8Token.safeTransfer(msg.sender, rewardInTX8ToTransfer);

        emit TicketsClaim(msg.sender, rewardInTX8ToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInTX8) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in TX8 for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].tx8PerBracket[j] =
                ...
                // A. No TX8 to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].tx8PerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInTX8 - amountToShareToWinners);

        // Transfer TX8 to treasury address
        tx8Token.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        tx8Token.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInTX8 += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInTX8 >= minpriceTicketInTX8) && (_priceTicketInTX8 <= maxpriceTicketInTX8),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInTX8: _priceTicketInTX8,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            tx8PerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInTX8: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(tx8Token), "Cannot be TX8 token");
    ...
    /**
     * @notice Set TX8 price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minpriceTicketInTX8: minimum price of a ticket in TX8
     * @param _maxpriceTicketInTX8: maximum price of a ticket in TX8
     */
    function setMinAndMaxTicketPriceInTX8(uint256 _minpriceTicketInTX8, uint256 _maxpriceTicketInTX8)
        external
        onlyOwner
    {
        require(_minpriceTicketInTX8 <= _maxpriceTicketInTX8, "minPrice must be < maxPrice");

        minpriceTicketInTX8 = _minpriceTicketInTX8;
        maxpriceTicketInTX8 = _maxpriceTicketInTX8;
    }
     ...
     * @param _priceTicket price of a ticket (in TX8)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].tx8PerBracket[_bracket];
        } else {
            return 0;
        }
    }
